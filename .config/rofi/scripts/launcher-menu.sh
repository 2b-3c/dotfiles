...
menu_setup_config() {
    ...
    case "$CHOICE" in
        ...
        *Waybar)   kitty --class "popup" --title "popup:󰍜 Waybar" sh -c "${EDITOR:-nvim} $HOME/.config/waybar/config.jsonc; pkill waybar 2>/dev/null; sleep 0.5; waybar &disown" ;;
    esac
}
...
menu_update_process() {
    ...
    case "$CHOICE" in
        ...
        *Hyprsunset) pkill hyprsunset 2>/dev/null; sleep 0.1; hyprsunset -t 4500 &disown; notify-send "Hyprsunset" "Restarted" ;;
        *rofi)       pkill rofi; notify-send "rofi" "rofi restarted" ;;
        *Waybar)     pkill waybar 2>/dev/null; sleep 0.5; waybar &disown; notify-send "Waybar" "Restarted" ;;
    esac
}
...
# ── About ────────────────────────────────────────────────────────
menu_about() {
    kitty --title "terminal" zsh -c "tput civis; fastfetch; read -r -s; tput cnorm"
}

...
# Usage: screenrecord.sh [--with-desktop-audio] [--with-microphone-audio] [--with-webcam] [--stop-recording]
...
stop_screenrecording() {
  ...
  if pgrep -f "^gpu-screen-recorder" >/dev/null; then
    ...
    (
      ACTION=$(notify-send "Screen recording saved" "Click to open in mpv" -t 10000 -i "${preview:-$filename}" -A "default=open")

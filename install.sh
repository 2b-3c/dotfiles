...
echo "vantablack" > "$CURRENT_DIR/theme.name"
...
# ══════════════════════════════════════════════════════════════════
# 17.1 — Apply Vantablack Theme
# ══════════════════════════════════════════════════════════════════
step "17.1 — Apply Vantablack Theme"

# Waybar
cp "$CONFIG_DST/waybar/themes/vantablack.css"        "$CONFIG_DST/waybar/theme.css"
ok "Waybar theme → Vantablack"

# Starship
cp "$CONFIG_DST/starship/themes/vantablack.toml"     "$HOME/.config/starship.toml"
ok "Starship theme → Vantablack"

# Kitty
cp "$CONFIG_DST/kitty/themes/vantablack.conf"        "$CONFIG_DST/kitty/theme.conf"
ok "Kitty theme → Vantablack"

# Rofi
cp "$CONFIG_DST/rofi/themes/vantablack.rasi"         "$CONFIG_DST/rofi/themes/theme.rasi"
ok "Rofi theme → Vantablack"

# SwayNC
cp "$CONFIG_DST/swaync/themes/vantablack.css"        "$CONFIG_DST/swaync/theme.css"
ok "SwayNC theme → Vantablack"

# btop
cp "$CONFIG_DST/btop/themes/vantablack.theme"        "$CONFIG_DST/btop/themes/theme.theme"
ok "btop theme → Vantablack"

# cava
sed -i '/^gradient_color/d; /^gradient_count/d; /^gradient$/d' "$CONFIG_DST/cava/config"
sed -i "/^\[color\]/a gradient_color_4 = '#ffffff'\ngradient_color_3 = '#8d8d8d'\ngradient_color_2 = '#3d3d3d'\ngradient_color_1 = '#141414'\ngradient_count = 4\ngradient = 1" "$CONFIG_DST/cava/config"
ok "cava theme → Vantablack"

# Yaru icon theme
sudo gtk-update-icon-cache /usr/share/icons/Yaru-dark 2>/dev/null || true
ok "Yaru icon theme configured (default: Yaru-bark-dark for Vantablack)"

# Wallpaper — save path to restore on first launch
VANTA_WALL="$HOME/Pictures/Wallpapers/themes/vantablack.jpg"
echo "$VANTA_WALL" > "$CONFIG_DST/hypr/.current_wallpaper"
ok "Default wallpaper → Vantablack"
...
cp "$CONFIG_DST/gtk-3.0/themes/vantablack.css" "$HOME/.config/gtk-3.0/gtk.css"
cp "$CONFIG_DST/gtk-4.0/themes/vantablack.css" "$HOME/.config/gtk-4.0/gtk.css"
cp "$CONFIG_DST/gtk-3.0/themes/vantablack.ini" "$HOME/.config/gtk-3.0/settings.ini"
cp "$CONFIG_DST/gtk-4.0/themes/vantablack.ini" "$HOME/.config/gtk-4.0/settings.ini"
gsettings set org.gnome.desktop.interface gtk-theme    "Adwaita-dark"           2>/dev/null || true
gsettings set org.gnome.desktop.interface icon-theme   "Yaru-bark-dark"         2>/dev/null || true
gsettings set org.gnome.desktop.interface cursor-theme "Bibata-Modern-Classic"  2>/dev/null || true
gsettings set org.gnome.desktop.interface cursor-size  16                       2>/dev/null || true
